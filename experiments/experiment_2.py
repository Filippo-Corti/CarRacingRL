...
The actor width is fixed at Experiment 1's original medium `(64, 64)` policy.
The later tiny-actor extension does not alter this predeclared selection.
...
Two observations times ten roots is 20 runs of one million interactions.
"""
...
def specifications(scale: Scale) -> list[RunSpecification]:
    ...
    for name, observation in OBSERVATIONS.items():
        ...
        for root in scale.roots:
            run_id = f"ppo-medium-{name}-seed-{root}"
            ...
            runs.append(
                RunSpecification(
                    ...
                    _run_contract(scale, execution),
                )
            )
    return runs


def _run_contract(scale: Scale, execution: ExecutionConfig) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "training.actor": replace(
            MEDIUM_ACTOR_CONFIG, learning_rate=ACTOR_LEARNING_RATE
        ).to_dict(),
        "training.critic": replace(
            FIXED_CRITIC_CONFIG, learning_rate=CRITIC_LEARNING_RATE
        ).to_dict(),
        "training.execution.environment_workers": execution.environment_workers,
        "training.ppo": PPOConfig().to_dict(),
        "training.logging.final_evaluation_trajectory_circuits": (
            FINAL_EVALUATION_TRAJECTORY_CIRCUITS
        ),
    }
