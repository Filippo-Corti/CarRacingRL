...
class ActorConfig(SerializableConfig):
    ...
    Lowering the cap further does not help and was tried. At `-0.3`, a scale of
    `0.74`, PPO pressed against that bound instead and its finished policy still
    alternated between control limits, so the cap moves where the dispersion
    saturates without removing the pressure that takes it there. Only PPO ever
    approaches a bound; REINFORCE and A2C settle near `0.58` unaided.
