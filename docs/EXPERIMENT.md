...
$3$ algorithms and $3$ network sizes are compared, for a total of $3 \times 3=9$ training configurations.

Each training configuration is run $5$ times, using $5$ different root identities.
The total number of training runs is therefore $3 \times 3 \times 5 = 45$:

| Algorithm \ Size | `(32, 32)` actor | `(64, 64)` actor | `(256, 256)` actor |
|---|---:|---:|---:|
...
* A checkpoints save every $250\,000$ interactions and at the end.

### Definition of Convergence

The algorithm reaches convergence when $3$ consecutive evaluations (run over a span of $150\,000$ interactions) all complete a full lap, in at most $34$ seconds.
...
Each training configuration is run $5$ times, using $5$ different root identities.
The total number of training runs is therefore $2 \times 5 = 10$:
...
- A fixed PPO architecture, selected based on the results of Experiment 1;
...
The selection of the PPO Architecture for Experiment 2 is performed following these steps, considering the Experiment 1 results:
...
These criteria define the cost we accept to pay to avoid an architecture with too many parameters.
In other words, a large network cannot be just moderately better than a small one: in order to be chosen, it must be considerably better, as its cost is considerably more.  

### Definition of Convergence 

The algorithms reaches convergence when $3£ consecutive evaluations (run over a span of $150\,000$ interactions):
* Have a validation completion rate of at least `0.75`, meaning that they successfully complete $12$ out of the $16$ circuits.
* Have a median validation normalized progress of at least `0.95`, ensuring that non-completed circuits show progress anyways.
